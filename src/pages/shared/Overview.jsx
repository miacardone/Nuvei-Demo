...
/**
 * Landing page for the acquirer and issuer perspectives — the counterpart to
 * the merchant Dashboard. Same case book, same analytics (domain/metrics.js),
 * re-framed: an acquirer's risk team and an issuer's fraud team read the same
 * chargeback activity differently, but it is still one book, not two.
 */
...
const COPY = {
  acquirer: {
    ...
    description: `${ACQUIRER_NAME}'s live dispute position across the merchant portfolio — volumes, reason-code mix and risk throughput.`,
    activityTitle: 'Portfolio Dispute Activity Per Week',
    dailyTitle: 'New Disputes Per Day',
  },
  issuer: {
    title: 'Issuer overview',
    description: `${ISSUING_BANK.name}'s live dispute position across cardholder authorizations — volumes, reason-code mix and fraud throughput.`,
    activityTitle: 'Chargeback Activity Per Week',
    dailyTitle: 'New Chargebacks Per Day',
  },
};
...
export function Overview() {
  ...
  const typeSplit = useMemo(() => [
    ...
    { label: brand.terms.claims, value: kpis.claims, color: 'var(--c-series-1)' },
  ], [brand.terms, kpis]);
    ...
    <>
      ...
      <div className="stack">
        <div className="grid grid--auto" style={{ gap: 'var(--s-3)' }}>
          <Kpi label="Open disputes" value={formatNumber(kpis.openCases)} meta={`${formatNumber(kpis.overdueCases)} overdue`} trend={openTrend} spark={openSpark} tooltip="New cases opened in the last 30 days vs. the 30 days before that." />
          ...
          <Kpi label={`${terms.chargebacks} : ${terms.claims}`} value={`${formatNumber(kpis.chargebacks)} : ${formatNumber(kpis.claims)}`} trend={mixTrend} spark={mixSpark} tooltip={`${terms.claims} opened in the last 30 days vs. the 30 days before that.`} />
        </div>

        <Card title={copy.activityTitle} description="Cases by their current status, grouped by the week they were created." action={<RangeChip value={range} onChange={setRange} />} bodyClassName="card__body--chart">
          <BarChart
            data={activity}
            height={200}
            series={[
        ...
        <Card title={copy.dailyTitle} description="New cases created each day over the selected range." action={<RangeChip value={range} onChange={setRange} />} bodyClassName="card__body--chart">
        ...
        <Card title={`${terms.chargebacks} vs. ${terms.claims} Trend`} description="Weekly volume split between the two intake paths." bodyClassName="card__body--chart">
          <LineChart
            data={typeTrend}
            height={200}
            series={[
              { key: 'chargeback', name: terms.chargebacks },
              { key: 'claim', name: terms.claims, color: 'var(--c-series-1)' },
            ]}
          />
        </Card>

        <Card title={`Average Handle Time by ${terms.analyst}`} description="Mean minutes spent per case, by analyst. Dashed line marks the team average." bodyClassName="card__body--chart">
        ...
        <Card title={`${terms.analyst} activity`} bodyClassName="card__body--flush">
          <DataTable columns={analystColumns} rows={analysts.slice(0, 6)} rowKey={(r) => r.email} density="comfortable" />
        </Card>
      </div>
    </>
  );
